...
while (<>) 
{
    ...
    # Convert existing number specifiers into written form
    s/(\d+)([A-Z]+)?/\1\#\2/g;
    ...
    s/Þ/p/g;
    s/ß/b/g;
    #and they should be lower case too, so commented the following out and added the above
    #s/À/A/g;
    #s/Á/A/g;
    #s/Â/A/g;
    #s/Ã/A/g;
    ...
    s/(\d+)\s*\.\s*(tammi|helmi|maalis|huhti|touko|kesä|heinä|elo|syys|loka|marras|joulu)/$1\#JNOM $2/gi;
    ...
    s/[^abcdefghijklmnopqrstuvwxyzåäö.?!]/ /g;
    #Remove empty lines
    s/^*\n//;
    s/^\s+//;
    s/\s+$//;
